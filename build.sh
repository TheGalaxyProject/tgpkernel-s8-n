...
if [ $N8 == "1" ]; then
	sudo cp -rf $RDIR/.build/ramdisk-n8/* $WORKDIR/ramdisk 
fi
}
...
LOGFILE=$OUTPUT/build-s8-s8.log
BUILD_PREPARE
FUNC_BUILD_BOOTIMG
BUILD_CLEAN
...
export ANDROIDVERSION=N8.Port
export KERNELTITLE=$KERNELNAME.$ANDROIDVERSION.$VERSION_NUMBER
MODEL=dreamlte
LOGFILE=$OUTPUT/build-s8-n8.log
BUILD_PREPARE
FUNC_BUILD_BOOTIMG
BUILD_CLEAN
...
ZIP_DATE=`date +%Y%m%d`
ZIP_NAME=$KERNELNAME.G95xx.N.v$VERSION_NUMBER.$ZIP_DATE.EOL.zip
